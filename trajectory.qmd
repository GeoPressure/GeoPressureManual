...
In GeoPressureR, a `path` is a standard data.frame created and used [in several other functions](https://geopressure.org/GeoPressureR/reference/index.html#path).
...
You might want to save some of the key objects such as `tag` or `marginal` in an `R` format. Following the default structure described in [GeoPressureTemplate](https://github.com/GeoPressure/GeoPressureTemplate#project-structure-file_folder), you should save them in the `./data/interim` folder.
...
This workflow is described in detail in [GeoPressureTemplate workflow](geopressuretemplate-workflow.html).
:::
