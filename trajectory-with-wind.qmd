...
```{r 05-tag-geopressure-light-create}
#| cache: true
#| code-fold: true
tag <- tag_create(
  "18LX",
...
If you have many tracks to download and do not want to block your console, you can run this step asynchronously in a separate R process. RStudio Jobs and packages such as `job` or `mirai` can help with this.
...
```{r 05-map-marginal-plot}
plot(marginal, path = path_most_likely)
```

```{r 05-path-simulation-plot}
...
```{r 05-edge-speed-plots}
#| echo: false
edge_simulation$name <- glue::glue(
  "{edge_simulation$stap_s}->{edge_simulation$stap_t}"
)
