...
Use the timeseries that provides the relevant information:

- **With acceleration data:** apply `flight` to the acceleration timeseries. On the pressure timeseries, use `discard` for unusable pressure measurements and `elev_*` for changes in elevation. No need to mark pressure as `discard` solely because the acceleration data indicates flight.
- **Without acceleration data:** apply `flight` and, where needed, `discard` or `elev_*` directly to the pressure timeseries.
:::
